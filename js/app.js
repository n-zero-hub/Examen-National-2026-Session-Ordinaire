...
function buildQuestionGrid() {
  ...
  QUESTIONS.forEach((q, i) => {
    if (q.section !== lastSection) {
      ...
      label.textContent =
        q.section === "chimie" ? "⚗️ Chimie (1–14)" : "⚡ Physique (15–40)";
...
function updateSectionTabs(active) {
  ...
  document
    .getElementById("tab-chimie")
    .classList.toggle("active", cur === "chimie");
  document
    .getElementById("tab-physique")
    .classList.toggle("active", cur === "physique");
}
...
function updateSectionBadges() {
  const chDone = QUESTIONS.slice(0, 14).filter(
    (_, i) => state.answers[i] !== null,
  ).length;
  const phDone = QUESTIONS.slice(14).filter(
    (_, i) => state.answers[i + 14] !== null,
  ).length;
...
async function sendResults() {
  ...
  const wrong = state.answers.filter(
  ...
  try {
    await fetch(
      "https://script.google.com/macros/s/AKfycbyMXsl0RdGsYij43AArHtDeTQrcclHiH6a25g4f6iyIAaMBop4oLvAsI-LvmYnFrE-G/exec",
      {
        method: "POST",
        headers: {
...
function showCorrection() {
  ...
  const subsections = {};
  QUESTIONS.forEach((q, i) => {
    if (!subsections[q.subsection])
      subsections[q.subsection] = { section: q.section, qs: [] };
